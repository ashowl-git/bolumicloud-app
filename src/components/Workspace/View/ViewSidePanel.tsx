    ...
    >
      ...
      <WorkspacePanelSection title="위치" icon={<MapPin size={14} />}>
        <div className="flex flex-wrap gap-1.5 mb-3">
          {CITY_PRESETS.map((city, idx) => (
            <button
              ...
              disabled={disabled}
              className="border border-gray-200 dark:border-slate-700 hover:border-red-600/30 px-2 py-1
                text-xs text-gray-700 dark:text-slate-300 hover:text-red-600 transition-all disabled:opacity-50"
            >
              {city.name.ko}
            </button>
          ))}
        ...
        <div className="grid grid-cols-3 gap-2">
          <div>
            <label className="text-[10px] text-gray-500 dark:text-slate-400 block mb-0.5">위도</label>
            <input
              type="number" step="0.001" value={config.latitude}
              onChange={(e) => onConfigChange({ latitude: Number(e.target.value) })}
              disabled={disabled}
              className="w-full border border-gray-200 dark:border-slate-700 px-2 py-1 text-xs focus:outline-none focus:border-red-600/30 disabled:opacity-50"
            />
          </div>
          <div>
            <label className="text-[10px] text-gray-500 dark:text-slate-400 block mb-0.5">경도</label>
            <input
          ...
          <div>
            <label className="text-[10px] text-gray-500 dark:text-slate-400 block mb-0.5">자오선</label>
            <input
      ...
      <WorkspacePanelSection title="반구 설정" icon={<Eye size={14} />}>
        <div className="mb-3">
          <label className="text-[10px] text-gray-500 dark:text-slate-400 block mb-1">반구 해상도</label>
        ...
        <div>
          <label className="text-[10px] text-gray-500 dark:text-slate-400 block mb-1">투영 방식</label>
                  ...
                  <button
                    ...
                    className={`flex-1 py-2 text-xs text-center transition-all rounded disabled:opacity-50 ${
                      isSelected
                        ...
                        : 'border border-gray-200 dark:border-slate-700 text-gray-700 dark:text-slate-300 hover:border-red-600/30 hover:bg-gray-50/50'
                    }`}
                  >
                    {label.ko}
                  </button>
      ...
      <WorkspacePanelSection title="관찰점" icon={<List size={14} />} badge={points.length} defaultOpen={points.length > 0}>
        {points.length === 0 ? (
          <div className="py-3 text-center">
            <p className="text-xs text-gray-500 dark:text-slate-400">벽면 또는 지붕을 클릭하여 관찰점을 배치하세요</p>
            <p className="text-[10px] text-gray-300 dark:text-slate-600 mt-1">도구 모음에서 배치 모드(P)를 선택한 후 클릭</p>
          </div>
        ) : (
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-2 py-1.5 text-xs transition-colors rounded flex items-center justify-between ${
                    selectedPointId === pt.id ? 'bg-red-50 text-red-600' : 'text-gray-600 dark:text-slate-300 hover:bg-gray-50'
                  }`}
                >
                  <span>{pt.name}</span>
          ...
          <WorkspacePanelSection title="결과 요약 (SVF)" icon={<BarChart3 size={14} />}>
            <div className="flex items-center gap-2 mb-2">
              <div className="flex items-center gap-1 text-[10px]">
                ...
                <span className="text-gray-400 dark:text-slate-500">높음</span>
              </div>
              <div className="flex items-center gap-1 text-[10px]">
                <span className="w-2 h-2 rounded-full bg-amber-500" />
                <span className="text-gray-400 dark:text-slate-500">중간</span>
              </div>
              <div className="flex items-center gap-1 text-[10px]">
                <span className="w-2 h-2 rounded-full bg-red-500" />
                <span className="text-gray-400 dark:text-slate-500">낮음</span>
              </div>
            </div>
