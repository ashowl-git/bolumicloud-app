  ...
  return (
    <div>
      <button
        type="button"
        onClick={onToggle}
        className="text-[11px] text-gray-500 dark:text-slate-400 hover:text-gray-700 underline underline-offset-2"
      >
        {showReference
          ? '유리 광학 특성 레퍼런스 접기'
          : '유리 광학 특성 레퍼런스 보기'}
      ...
      {showReference && (
        <div className="mt-2 border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 overflow-x-auto">
          <table className="w-full text-[10px]">
            <thead>
              <tr className="border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800">
                <th className="text-left p-1.5 font-medium text-gray-600 dark:text-slate-300">유리 종류</th>
                ...
                <th className="text-center p-1.5 font-medium text-gray-600 dark:text-slate-300">SHGC</th>
                <th className="text-center p-1.5 font-medium text-gray-600 dark:text-slate-300">현휘</th>
              </tr>
            </thead>
            <tbody>
              {ENHANCED_GLASS_PRESETS.map((p) => {
                ...
                return (
                  <tr
                    key={p.label}
                    className="border-b border-gray-100 dark:border-slate-800 hover:bg-gray-50 cursor-pointer"
                    onClick={() => onApplyPreset(p)}
                  >
                    <td className="p-1.5 text-gray-700 dark:text-slate-300">
                      <span className="text-[9px] text-gray-400 dark:text-slate-500 mr-1">
                        [{GLASS_CATEGORY_LABELS[p.category]}]
                      </span>
                      {p.label}
                    </td>
                    <td className="text-center p-1.5 text-gray-600 dark:text-slate-300 font-mono">
                      {(p.vlt * 100).toFixed(0)}
                    </td>
