    ...
    <div className="space-y-4">
      ...
      <div className="border border-red-200 bg-red-50 p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h3 className="text-sm font-medium text-gray-900 dark:text-slate-100">원인 분석</h3>
        </div>
        <div className="grid grid-cols-3 gap-4">
          <div>
            <p className="text-xs text-gray-500 dark:text-slate-400">부적합 측정점</p>
            <p className="text-2xl font-light text-red-600">
              {causeResult.total_non_compliant}
            </p>
          </div>
          <div>
            <p className="text-xs text-gray-500 dark:text-slate-400">원인 건물 수</p>
            <p className="text-2xl font-light text-gray-900 dark:text-slate-100">
              {rankedBuildings.length}
            </p>
          </div>
          <div>
            <p className="text-xs text-gray-500 dark:text-slate-400">전체 건물 수</p>
            <p className="text-2xl font-light text-gray-900 dark:text-slate-100">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 dark:bg-slate-800 border-b border-gray-200 dark:border-slate-700">
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400">건물</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400">높이</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400">총 차단(분)</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 dark:text-slate-400">영향 측정점</th>
              </tr>
            </thead>
            <tbody>
              {rankedBuildings.map((building) => {
                ...
                return (
                  <tr
                    ...
                    className={`border-b border-gray-100 dark:border-slate-800 cursor-pointer transition-colors ${
      ...
      {/* 측정점별 상세 */}
      <details className="border border-gray-200 dark:border-slate-700">
        <summary className="px-4 py-3 text-sm text-gray-700 dark:text-slate-300 cursor-pointer hover:bg-gray-50">
          ...
          {causeResult.point_causes.map((pc) => (
            <div key={pc.point_id} className="px-4 py-3">
              <div className="flex items-center justify-between mb-1">
                <span className="text-sm font-medium text-gray-900 dark:text-slate-100">{pc.point_name}</span>
