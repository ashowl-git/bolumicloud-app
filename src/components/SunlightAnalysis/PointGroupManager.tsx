    ...
    <button
      ...
      className={`w-full flex items-center gap-1 text-left px-2 py-1 text-xs
        ...
        isSelected
          ...
          : 'text-gray-600 dark:text-slate-300 hover:bg-gray-50'
      }`}
    >
      {sorted && (
        <>
      ...
      <span className={`text-[10px] tabular-nums ${
        isSelected ? 'text-red-400' : 'text-gray-500 dark:text-slate-400'
      }`}>
          ...
          <div className="space-y-2">
            ...
            <div className="flex items-center gap-1.5">
              <input
                ...
                className="flex-1 border border-gray-200 dark:border-slate-700 px-2 py-1 text-xs
                  focus:outline-none focus:border-red-600/30"
                disabled={disabled}
              />
              <button
                onClick={() => onSortGroup(activeGroup.id)}
                className="p-1.5 border border-gray-200 dark:border-slate-700 text-gray-500 dark:text-slate-400 hover:text-red-600
              ...
              <button
                ...
                className={`p-1.5 border transition-colors ${
                  activeGroup.reverseColumns
                    ...
                    : 'border-gray-200 dark:border-slate-700 text-gray-500 dark:text-slate-400 hover:text-red-600 hover:border-red-600/30'
                }`}
                title="열 순서 반전"
              >
              ...
              {groups.length > 1 && (
                <button
                  ...
                  className="p-1.5 border border-gray-200 dark:border-slate-700 text-gray-400 dark:text-slate-500 hover:text-red-500
            ...
            {activeGroup.points.length === 0 ? (
              <div className="flex flex-col items-center gap-2 py-3 text-center">
                <Crosshair size={18} className="text-gray-300 dark:text-slate-600" />
              ...
              <div>
                {/* 헤더 */}
                {activeGroup.sorted && (
                  <div className="flex items-center gap-1 px-2 py-1 text-[10px] text-gray-500 dark:text-slate-400
                    border-b border-gray-100 dark:border-slate-800 sticky top-0 bg-white dark:bg-slate-800">
