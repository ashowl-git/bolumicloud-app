        ...
        {items.map((item) => (
          <label key={item.label} className="flex items-center gap-1.5 cursor-pointer">
            <input
              ...
              className="rounded border-gray-300 dark:border-slate-600 text-red-600 focus:ring-red-500 w-3 h-3"
            />
            <span
            ...
            <span className="text-gray-400 dark:text-slate-500 ml-auto">{item.levels}</span>
          </label>
        ))}
      </div>
    </div>
