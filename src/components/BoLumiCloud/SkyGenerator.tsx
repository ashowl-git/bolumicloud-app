      ...
      >
        ...
        <div className="border border-gray-200 dark:border-slate-700 p-6 mb-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-sm text-gray-700 dark:text-slate-300 mb-2 block">월 (Month)</label>
              <select
                value={month}
                onChange={(e) => setMonth(Number(e.target.value))}
                className="w-full border border-gray-200 dark:border-slate-700 px-4 py-2 text-sm focus:border-red-600/30 transition-colors"
              >
            ...
            <div>
              <label className="text-sm text-gray-700 dark:text-slate-300 mb-2 block">일 (Day)</label>
              <input
                type="number"
                min="1"
                max="31"
        ...
        <div className="border border-gray-200 dark:border-slate-700 p-6 mb-6">
          ...
          {/* 주요 도시 프리셋 */}
          <div className="mb-4">
            <label className="text-sm text-gray-700 dark:text-slate-300 mb-2 block">주요 도시</label>
            <div className="flex flex-wrap gap-2">
              {cityPresets.map((city) => (
                <button
                  key={city.name}
                  onClick={() => handleCitySelect(city)}
                  className="border border-gray-200 dark:border-slate-700 hover:border-red-600/30 px-4 py-2 text-sm
                    text-gray-900 dark:text-slate-100 hover:text-red-600 transition-all"
                >
                  {city.name}
                </button>
              ))}
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="text-sm text-gray-700 dark:text-slate-300 mb-2 block">위도 (Latitude)</label>
              <input
                type="number"
                step="0.0001"
                value={latitude}
            ...
            <div>
              <label className="text-sm text-gray-700 dark:text-slate-300 mb-2 block">경도 (Longitude)</label>
              <input
                type="number"
                step="0.0001"
                value={longitude}
            ...
            <div>
              <label className="text-sm text-gray-700 dark:text-slate-300 mb-2 block">표준 자오선</label>
              <input
                type="number"
                value={meridian}
        ...
        <div className="mt-8 p-6 border border-gray-200 dark:border-slate-700">
          <h4 className="font-medium text-gray-900 dark:text-slate-100 mb-3">gendaylit 기능 설명</h4>

          <div className="space-y-3 text-sm text-gray-800 dark:text-slate-200">
            <p>
              <span className="font-medium text-gray-900 dark:text-slate-100">CIE 표준 하늘:</span>
              국제조명위원회(CIE)의 표준 주광 모델을 생성합니다.
              Perez All-Weather 알고리즘 기반으로 맑음/흐림 상태를 자동 계산합니다.
            </p>
            <p>
              <span className="font-medium text-gray-900 dark:text-slate-100">사용 용도:</span>
              Radiance 시뮬레이션의 광원 파일(.rad)로 사용됩니다.
              rtrace, rpict 등과 함께 사용하여 실제 일조 분석을 수행할 수 있습니다.
            </p>
            <p>
              <span className="font-medium text-gray-900 dark:text-slate-100">파라미터:</span>
            </p>
            <ul className="list-disc list-inside space-y-1 text-xs text-gray-800 dark:text-slate-200 ml-4">
